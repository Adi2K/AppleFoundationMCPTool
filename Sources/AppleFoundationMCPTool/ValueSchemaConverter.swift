...
//
// Inspired by : 
// https://github.com/0Itsuki0/Swift_FoundationModelsWithMCP/blob/main/FoundationModelWithMCP/ValueSchemaConvertor.swift
//
import Foundation
...
class ValueSchemaConverter {
  ...
  private func propertyFrom(
    ...
    case JSONSchemaKey.number.rawValue:
      let result = applyEnumAndConstConstraints(
        for: value,
        name: propertyName,
        description: propertyDescription,
        defaultSchema: DynamicGenerationSchema(type: Double.self),
        required: isRequired
      )
      return property(
        name: propertyName,
    ...
    case JSONSchemaKey.integer.rawValue:
      let result = applyEnumAndConstConstraints(
        ...
        name: propertyName,
        description: propertyDescription,
        defaultSchema: DynamicGenerationSchema(type: Int.self),
        required: isRequired
      )
      return property(
        name: propertyName,
    ...
    case JSONSchemaKey.string.rawValue:
      let result = applyEnumAndConstConstraints(
        ...
        name: propertyName,
        description: propertyDescription,
        defaultSchema: DynamicGenerationSchema(type: String.self),
        required: isRequired
      )
      return property(
        name: propertyName,
    ...
    default:
      ...
      if let enumValue = value[JSONSchemaKey.enumType.rawValue] {
        ...
        return property(
          ...
          description: propertyDescription,
          schema: result.schema,
          required: result.isRequired ?? isRequired
        )
      }
  ...
  func schema(from object: [String: Value]) -> DynamicGenerationSchema {
    ...
    if let type = getPropertyTypeString(object: object) {
      ...
      case JSONSchemaKey.number.rawValue:
        return applyEnumAndConstConstraints(
          for: object,
          name: title,
  ...
  private func applyEnumAndConstConstraints(
    for value: [String: Value],
    name: String,
    description: String?,
    defaultSchema: DynamicGenerationSchema,
    required: Bool
  ) -> (schema: DynamicGenerationSchema, isRequired: Bool) {
