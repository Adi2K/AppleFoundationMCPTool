...
final class ValueSchemaConverterTests: XCTestCase {
  ...
  func testObjectWithStringIntBoolProperties() throws {
    ...
    let generationSchema = try GenerationSchema(root: XCTUnwrap(result), dependencies: [])
  ...
  func testNullableFieldViaAnyOf() {
    let schema: [String: Value] = [
      "type": .string("object"),
      "properties": .object([
        "nickname": .object([
  ...
  func testConnectAndDiscoverToolsViaInMemory() async throws {
    ...
    await server.withMethodHandler(CallTool.self) { request in
      if request.name == "get_weather" {
        ...
        return CallTool.Result(content: [.text("Sunny, 72\u{00B0}F in \(city)")])
      }
